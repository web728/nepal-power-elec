...
import { postShowReportSchema, type PostShowReportInput } from "@/lib/validations/forms";
...
const initialValues: PostShowReportInput = {
  fullName: "",
  email: "",
  phone: "",
  country: "",
...
type Errors = Partial<Record<keyof PostShowReportInput, string>> & {
  recaptcha?: string;
};

export function PostShowReportForm() {
  const [values, setValues] = useState<PostShowReportInput>(initialValues);
  ...
  function update<K extends keyof PostShowReportInput>(key: K, value: PostShowReportInput[K]) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    // 1. Validate Form
    const result = postShowReportSchema.safeParse(values);
    ...
    if (!result.success) {
      ...
      for (const key in fieldErrors) {
        ...
        if (messages && messages.length > 0) {
          nextErrors[key as keyof PostShowReportInput] = messages[0];
        }
      }
    }
    ...
    try {
      const res = await fetch("/api/post-show-report", {
        method: "POST",
      ...
      showToast("Verification successful! Your download is ready.", "success");
      trackEvent(AnalyticsEvents.CONTACT_SUBMIT, { category: "PostShowReport" });

      // Auto trigger download
      if (data.downloadUrl) {
        const link = document.createElement("a");
        link.href = data.downloadUrl;
        link.download = "Nepal-Electric-Power-Lights-Expo-2025-Post-Show-Report.pdf";
        link.target = "_blank";
  ...
  if (downloadUrl) {
    return (
      <div className="rounded-2xl border border-emerald-300 bg-white p-8 text-center shadow-lg">
        ...
        <p className="text-sm font-bold uppercase tracking-wider text-emerald-600">Report Unlocked</p>
        <p className="mt-3 text-base leading-relaxed text-slate-800">
          Thank you for registering. Your download should begin automatically. If it didn&apos;t start, click the button below.
        </p>
        <div className="mt-6 flex justify-center">
          <Button href={downloadUrl} target="_blank" rel="noopener noreferrer" variant="primary" size="lg">
            Download 2025 Post-Show Report (PDF)
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-xl">
      <h3 className="mb-2 text-xl font-bold tracking-tight text-slate-900">
        Fill Details to Download Report
      </h3>
      <p className="mb-6 text-sm text-slate-600">
        Please complete the brief form below to get instant access to the official 2025 post-show report.
      </p>
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
        ...
        <Button
          ...
          variant="cta-submit"
        >
          {isSubmitting ? "Processing…" : "Submit & Download Report"}
        </Button>
      </form>
    </div>
  );
